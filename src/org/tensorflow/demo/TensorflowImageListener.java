...
public class TensorflowImageListener implements OnImageAvailableListener {
  ...
  public void onImageAvailable(final ImageReader reader) {
    ...
    handler.post(
		...
		if(results != null){
			camFrag.detectedImage(!results.isEmpty());
			if(!results.isEmpty()){
				camFrag.setSearch(results.get(0).getTitle());
			}
		}
            computing = false;
          }
